...
function comicHtml(s){{if(s.comic_art)return `<div class="comic"><h3>Production comic: ${{esc(s.title)}}'s day</h3><img src="${{esc(s.comic_art)}}" alt="${{esc(s.title)}} comic"><div class="comic-note">Production image-generated comic from the day-story arc.</div></div>`;return fallbackComic(s)}}
...
function playAudio(){{const a=document.querySelector('.slide.active .deckAudio');if(!a)return;a.currentTime=0;a.play().catch(()=>alert('Browser blocked autoplay. Press play on the audio control.'))}}function stopAudio(){{const a=document.querySelector('.slide.active .deckAudio');if(a){{a.pause();a.currentTime=0}}}}
